...
// Check if the concatenated value is set in the URL
if(isset($_GET['concatenatedValue'])) {
    $concatenatedValue = $_GET['concatenatedValue'];
    
    // Split the concatenated value into name and roll number
    ...
    $query = "SELECT creditS, university_exam_results FROM $concatenatedValue "; // Replace CGP with your actual table name
    ...
    $sql_u = "SELECT COUNT(*) AS num_u FROM $concatenatedValue WHERE university_exam_results = 'U'";
    $result_u = $conn->query($sql_u);
    $num_u = ($result_u->num_rows > 0) ? $result_u->fetch_assoc()['num_u'] : 0;

    // Fetch attendance data
    $sql_attendance = "SELECT SUM(attend1) AS total_attend1, SUM(attend2) AS total_attend2 FROM $concatenatedValue";
...
<body>
    ...
    <div class="sem2table">
        <center>
            <form method="post" action="newfetch.php">
                <table border="2" class="sem2content">
                    ...
                    <?php
                    // Output table rows with data fetched from the database
                    if (isset($concatenatedValue)) { // Check if table name is set
                        $conn = new mysqli($servername, $username, $password, $dbname);
                        if ($conn->connect_error) {
                            die("Connection failed: " . $conn->connect_error);
                        }
                        $sql = "SELECT * FROM $concatenatedValue";
    ...
    <div class="foot2">
        <form class="sem2track">
            <tr>
                ...
                <td><input type="TEXT" value="<?php echo isset($num_u) ? $num_u : ''; ?>"></td>
            </tr>
            <br>
            <br>
            <br>
            <tr>
                ...
                <td><input type="text" value="<?php echo isset($attendance_percentage) ? $attendance_percentage : ''; ?>%"></td>
            </tr>
            <br>
            <br>
            <br>
            <tr>
                <td><h2>GPA:</h2></td> <br>
                <td><input type="text" value="<?php echo isset($gpa) ? $gpa : ''; ?>"></td>
            </tr>
            <br>
            <br>
            <br>
